...
function run_gaussian_sigma_sweep(sigma_values::AbstractVector)
    ...
    for sigma in sigma_values
        ...
        best_mst, mst_ari         = best_single_linkage_threshold(mst_complete, n_pts, ground_truth, 0.1)
        best_naive, naive_ari     = best_single_linkage_threshold(naive_st_complete, n_pts, ground_truth, .1)
        best_approx, approx_ari   = best_single_linkage_threshold(all_ST_approx_edges, n_pts, ground_truth, .1)
        best_optimal, optimal_ari = best_single_linkage_threshold(all_ST_optimal_edges, n_pts, ground_truth, .1)
        best_simple, simple_ari   = best_single_linkage_threshold(all_ST_simple_edges, n_pts, ground_truth, .1)
